...
#README: I collaborated on this assignment with Karthik Krishnan. Submissions are identical.
...
def parse_args():
    #define new parser object
    parser = argparse.ArgumentParser()

    #add two positional arguments: input filename and output filename
    parser.add_argument("InputFile", type=str, help="Enter name of input data file")
    parser.add_argument("OutputFile", type=str, help="Enter name of output data file")

    #get and return args passed in
...
def read_data(input_file):
    #read in excel sheets as dataframes using pandas excel methods
    dt_well = pd.read_excel(input_file, sheet_name="WellData")
    dt_dictionary = pd.read_excel(input_file, sheet_name="Dictionary")
...
def calculate_ratio(dt_well):
    dt_well_sl = dt_well[dt_well["TargetType"] == "Ch1Unknown"].merge(dt_well[dt_well["TargetType"] == "Ch2Unknown"], left_on="Well", right_on="Well")
    dt_well_sl.insert(5, "Ratio",dt_well_sl["Concentration_y"]/(dt_well_sl["Concentration_y"] + dt_well_sl["Concentration_x"]), True)
    return dt_well_sl
...
def master_merge(dt_dictionary, dt_well_sl):
    return dt_dictionary.merge(dt_well_sl, left_on="Well", right_on="Well")
...
def pivot(dt_master):
    print(pd.pivot_table(dt_master, index=["Strain1", "Strain2", "Replicate"], columns="Time Point", values="Ratio"))
    return pd.pivot_table(dt_master, index=["Strain1", "Strain2", "Replicate"], columns="Time Point", values="Ratio")
...
def strain_avg(dt_output1):
    return pd.pivot_table(dt_output1, index=["Strain1", "Strain2"], values=[2, 4, 6], aggfunc={2 : "mean", 4 : "mean", 6 : "mean"})
"""
Part VII: write a function that takes in an output file name, dt_output1, and dt_output2, and writes the two DataFrames
to the provided output file, with dt_output1 on a page named 'All_data', and dt_output2 on a page called 'Average_data',
