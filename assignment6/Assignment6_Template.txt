...
Editors: Lauren Sabo and Karthik Krishnan
"""
...
def open_pdb_files(ref_pdb_file, mov_pdb_file):
    ...
    ref_pdb = pdbp.get_structure('GluRecA', ref_pdb_file)
    mov_pdb = pdbp.get_structure('GluRecB', mov_pdb_file)
...
def create_polypeptides(ref_pdb, mov_pdb):
    ...
    ref_peptides = ppb.build_peptides(ref_pdb)
    mov_peptides = ppb.build_peptides(mov_pdb)
...
def create_pairwise_alignment(ref_seq, mov_seq):
    ...
    gap_open = -10
    gap_extend = -0.5
    alns = pairwise2.align.globalds(ref_seq, mov_seq, matrix, gap_open, gap_extend)
    t_alignment = alns[0]
    return t_alignment


def create_structure_alignment(t_alignment, ref_pdb, mov_pdb):
    """
    ...
    """
    # your code here
    alignment = MultipleSeqAlignment([Bio.SeqRecord.SeqRecord(t_alignment[0]),Bio.SeqRecord.SeqRecord(t_alignment[1])])
    structure_alignment = Bio.PDB.StructureAlignment(alignment, ref_pdb, mov_pdb)
...
def create_atom_list(structure_alignment):
    ...
    #lists of ref and mov length of 664
    ref_atoms = []
    mov_atoms = []
    for duo in structure_alignment.duos:
        if duo[0] and duo[1]:
            flag = ret_transmembrane(duo[0].get_full_id()[3][1])

            if flag:
                ref_atoms += [duo[0]['CA'], duo[0]['C'], duo[0]['N'], duo[0]['O']]
                mov_atoms += [duo[1]['CA'], duo[1]['C'], duo[1]['N'], duo[1]['O']]
        else:
            continue
...
def avg_residue_distance(res1, res2):
    ...
    for i in range(4):
        total += abs(res1.child_list[i] - res2.child_list[i])
...
def min_residue_distance(res1, res2):
    ...
    for i in range(len(res1.child_list)):
        for j in range(len(res2.child_list)):
            curr = res1.child_list[i] - res2.child_list[j]
            if (curr < min_dist):
                min_dist = curr
...
if __name__ == '__main__':
    ...
    structure_alignment = create_structure_alignment(t_alignment, ref_pdb, mov_pdb)
